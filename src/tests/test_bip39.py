...
WORDLIST_PATH = path.join(DIR_PATH, "wordlists")


class TestBIP39(TestCase):
    def test_bip39(self):
        for lang, values in VECTORS.items():
            wordlist = WordList.read(path.join(WORDLIST_PATH, f"{lang}.txt"))
            word_sep = "\u3000" if lang == "japanese" else " "
            for [
                input_entropy,
                expected_mnemonic,
                expected_seed,
                ...
                *_,
                ...
                entropy = Entropy(bytes.fromhex(input_entropy))
                mnemonic = Mnemonic.new(wordlist, entropy)
                self.assertEqual(word_sep.join(mnemonic), expected_mnemonic)
                seed = Seed.new(mnemonic, passphrase="TREZOR")
                self.assertEqual(seed.blob.hex, expected_seed)
                master_private_key = MasterPrivateKey.new(seed, Net.MAINNET)
                self.assertEqual(
                    master_private_key.serialize(),
                    expected_private_key.encode(),
                )
